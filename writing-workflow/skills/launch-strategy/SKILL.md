...
> ⚠️ **数据时效性警告**：以下达标线为历史参考值，**不保证当前有效**。番茄算法每年调整多次，以下阈值可能在当前已不适用。执行前必须通过 WebSearch 搜索"番茄小说 首秀 数据达标 {当前年份}"核实最新标准。**决策建议**：以下表仅作方向参考（S > A > B > C 的排序逻辑通常稳定），但具体数字门槛请以实时搜索结果为准。切勿以此表的数字作为收入预期的依据。
