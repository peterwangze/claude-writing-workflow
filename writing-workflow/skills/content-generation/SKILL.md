...
> **原则：任何偏离都是灾难的起源。** 过程偏离随章节累积会指数级放大——第1章的微小偏离，到第100章会变成不可修复的设定崩塌。不允许任何"可接受"的偏离。

| 偏离度 | 判定 | 处理 |
|--------|------|------|
| 0% | ✅ 通过 | 继续 |
| >0% | 🚫 阻断 | 必须修正至0%或重新生成。**不提供"忽略并继续"选项。** |
...
六、平台规则合规（新增）
  - 章节字数偏差：[实际字数] vs [目标字数] = [±X%]（>±20%=警告，>±30%=不合规）
  - 钩子交付验证：细纲承诺[强/中/弱] → 正文实际交付[强/中/弱]（未交付=F4阻断）
  - 情感签名履约：计划[主情感+辅助] → 本章传递[检测到情感]（连续5章偏离=F4阻断）
...
出现以下任一情况时，本章判定为**硬失败**，必须重写或修正后重新审查。**不提供任何例外或"确认偏离"选项。**

1. 场景覆盖率 < 100% — 任何必写场景缺失
2. 偏离度 > 0% — 任何与细纲的偏差
...
> 零容忍原则：偏离不是可以"评估合理性"的事情——偏离就是偏离。如果细纲本身需要调整，先修正细纲，再生成正文。不允许"偏离后确认合理继续"。
...
1. 重新生成（推荐）
2. 修正偏离部分后重新提交零偏离自检
3. 若细纲本身需要调整：先回退到细纲阶段修改，再重新生成正文

> ⚠️ 不存在"偏离合理"的选项。如果细纲需要调整，先修正细纲。
```
